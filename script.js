  ...
  cardsEl.innerHTML = ranked.map((t, idx) => {
    ...
    return `
      <div class="team-card" style="border-top-color: ${t.color}">
        ...
        ${t.logo ? `<div class="team-card-logo"><img src="${t.logo}" alt="${t.name} Logo" /></div>` : ''}
        ...
        <div class="team-card-roster">
          <div class="team-card-roster-title">Kader (${teamPlayers.length})</div>
          <ul class="team-card-roster-list">
            ${playerList}
          </ul>
        </div>
      </div>
  ...
  const rosterCards = data.teams.map(team => {
    ...
    return `
      <article class="team-roster-card" style="border-top-color:${team.color}">
        ${team.logo ? `<div class="team-roster-logo"><img src="${team.logo}" alt="${team.name} Logo" /></div>` : ''}
...
function renderRunFeed() {
  ...
  function filterActivities() {
    ...
    const filteredActivities = sortedActivities.filter(activity => {
      // Get all players in this activity (main player + partners)
      const allPlayerIds = [activity.player, ...(activity.partners || [])];
      // Check if ANY player belongs to a selected team
      return allPlayerIds.some(playerId => checkedTeams.includes(getPlayer(playerId).team));
    });
  ...
  function renderActivityCards(activities) {
    ...
    feedEl.innerHTML = activities.map(activity => {
      const mainPlayer = getPlayer(activity.player);
      const mainTeam = getTeam(mainPlayer.team);
      ...
      // Get all teams and players involved
      const allTeams = [mainTeam];
      const allPlayers = [mainPlayer];
      let partnersList = 'Solo';
      
      if (activity.partners && activity.partners.length > 0) {
        partnersList = activity.partners.map(pid => getPlayer(pid).name).join(', ');
        activity.partners.forEach(partnerId => {
          const partner = getPlayer(partnerId);
          const partnerTeam = getTeam(partner.team);
          allTeams.push(partnerTeam);
          allPlayers.push(partner);
        });
      }
      ...
      if (allTeams.length > 1) {
        // Duo run - use two-part border
        cardClasses += ' duo-run';
        color1 = allTeams[0].color;
        color2 = allTeams[1] ? allTeams[1].color : allTeams[0].color;
      }
      
      // Team display with logos and all players
      let teamDisplay = '';
      let playersDisplay = '';
      
      if (allTeams.length > 1) {
        // Duo run - show all teams and players
        teamDisplay = allTeams.map((t, idx) => `<span class="team-pill" style="background:${t.color}; display: inline-flex; align-items: center; gap: 6px;">
          ${t.logo ? `<img src="${t.logo}" alt="${t.name}" style="width: 20px; height: auto; max-height: 16px; object-fit: contain;">` : ''}
          ${t.emoji} ${t.name}
        </span>`).join(' ');
        
        playersDisplay = allPlayers.map((p, idx) => `
          <div class="run-card-duo-player">
            <span class="run-card-duo-name">${p.name}</span>
          </div>
        `).join('');
      } else {
        // Solo run
        teamDisplay = `<span class="team-pill" style="background:${mainTeam.color}; display: inline-flex; align-items: center; gap: 6px;">
          ${mainTeam.logo ? `<img src="${mainTeam.logo}" alt="${mainTeam.name}" style="width: 20px; height: auto; max-height: 16px; object-fit: contain;">` : ''}
          ${mainTeam.emoji} ${mainTeam.name}
        </span>`;
      }
      ...
      return `
        <div class="run-card ${cardClasses}" style="--color-1: ${color1}; --color-2: ${color2}">
          ...
          <div class="run-card-content">
            ${allTeams.length > 1 ? `
              <div class="run-card-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 14px;">
            ...
            ` : `
              <div class="run-card-header">
                <div class="run-card-title">${mainPlayer.name}</div>
                <div class="run-card-distance">${distanceDisplay}</div>
              </div>
            `}
            ...
            <div class="run-card-player">
              <strong>Team${allTeams.length > 1 ? 's' : ''}:</strong> ${teamDisplay}<br>
              <strong>Partner:</strong> ${partnersList}
            </div>
          </div>
        </div>
      `;
